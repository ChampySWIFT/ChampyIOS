...
- (void)countFrom:(float)startValue
               to:(float)endValue
...
- (void)setTextValue:(float)value {
  if (self.attributedFormatBlock != nil) {
    ...
    self.text = self.formatBlock(round(value));
  } else {
    // check if counting with ints - cast to int
    if ([self.format rangeOfString:@"%(.*)d" options:NSRegularExpressionSearch]
                .location != NSNotFound ||
...
- (CGFloat)currentValue {
  ...
  return self.startingValue +
         (updateVal * (self.destinationValue - self.startingValue));
}
