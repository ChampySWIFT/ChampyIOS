...
public enum ScoreBorderGlowMode {
  case forward, reverse, constant, noGlow
}

@IBDesignable class ScoreBorder: UIView, CAAnimationDelegate {
  ...
  @IBInspectable open var glowMode: ScoreBorderGlowMode = .forward {
    didSet {
      progressLayer.glowMode = glowMode
    }
  }
  //    0.4
  @IBInspectable open var progressThickness: CGFloat = 0.8  {//Between 0 and 1
    didSet {
  ...
  //here // 0.5
  @IBInspectable open var trackThickness: CGFloat = 0.2 {//Between 0 and 1
    didSet {
  ...
  @IBInspectable open var trackColor: UIColor = UIColor.clear {
    didSet {
  ...
  fileprivate func setInitialValues() {
    ...
    setColors(UIColor(patternImage: UIImage(named: "ScoreHolder")!))
    //initial color
  }
  ...
  fileprivate class KDCircularProgressViewLayer: CALayer {
    ...
    var glowMode: ScoreBorderGlowMode!
    ...
    fileprivate struct GlowConstants {
      fileprivate static let sizeToGlowRatio: CGFloat = 0.00000
      static func glowAmountForAngle(_ angle: Int, glowAmount: CGFloat, glowMode: ScoreBorderGlowMode, size: CGFloat) -> CGFloat {
        switch glowMode {
        case .forward:
...
//      CGContextAddArc(imageCtx, CGFloat(size.width/2.0),CGFloat(size.height/2.0), arcRadius, fromAngle, toAngle, clockwise == true ? 1 : 0)
      ...
      if colorsArray.count > 1 {
        ...
        for color in rgbColorsArray {
          let colorComponents: UnsafePointer<CGFloat> = color.cgColor.components as! UnsafePointer<CGFloat>
