...
- (CATransform3D)transformForItemViewWithOffset:(CGFloat)offset {
  ...
  case iCarouselTypeInvertedRotary: {
    ...
    CGFloat spacing =
        [self valueForOption:iCarouselOptionSpacing withDefault:0.95];
    CGFloat arc =
        [self valueForOption:iCarouselOptionArc withDefault:M_PI * 0.8];
    ...
    if (_vertical) {
      return CATransform3DTranslate(transform, 1.0, radius * sin(angle),
                                    radius * cos(angle) - radius);
    } else {
      return CATransform3DTranslate(transform, radius * sin(angle), 1.0,
                                    radius * cos(angle) - radius);
    }
  }
  case iCarouselTypeCylinder:
...
- (UIView *)loadViewAtIndex:(NSInteger)index
  ...
  if (index < 0) {
    view = [_dataSource carousel:self
          placeholderViewAtIndex:(NSInteger)(
                                     ceil((CGFloat)_numberOfPlaceholdersToShow /
                                          2.0)) +
                                 index
